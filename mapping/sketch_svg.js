...
// =========================
let lastPaintTime = 0;
let eraseDuration = 2000; // 2 secondes en millisecondes

// =========================
// LASER POSITION
// =========================
let laserFound = false;
let detectedPixels = 0;  // DEBUG
...
//  }
  ...
  // Effacer le dessin après 2 secondes sans peinture
...
function detectRedLaser() {
  ...
  let bestY = -1;
  let detectedCount = 0;
  detectedPixels = detectedCount;  // DEBUG
  ...
  if (bestX >= 0 && detectedCount > 0 && detectedCount < 50000) {
...
function updateBrushPaintingLaser() {
  ...
  // Mettre à jour le timer de peinture
...
function drawHUD() {
  push();
  // Texte blanc avec ombre pour la lisibilité
  ...
  text("Pixels detectes : " + detectedPixels, 20, 50);  // DEBUG
  text("C = effacer", 20, 75);
  text("V = webcam on/off", 20, 100);
  text("H = HUD on/off", 20, 125);
  text("Z = zone detection on/off", 20, 150);
